...
void main(void)
{
    ...
    while(true)
    {
        ...
        if (!scanf_s("%d",&inpCommand)) {puts("Input Error"); continue;}
        switch(inpCommand)
        {
        default: 
...
void medicalInputv1 (student * source)
{
    while (true)
    {
        ...
        if(!scanf_s("%lf",&source[studentCount].medical.heightWeight.height)) {puts("Input Error"); continue;}
        printf("Weight: "); fflush(stdin);
        if(!scanf_s("%lf",&source[studentCount].medical.heightWeight.weight)) {puts("Input Error"); continue;}
        break;
    }
}
...
void medicalInputv2 (student * source)
{
    ...
    {
        printf("Height: "); fflush(stdin);
        if(!scanf_s("%lf",&source[studentCount].medical.heightWeightPulsPressure._height)) {puts("Input Error"); continue;}
        printf("Weight: "); fflush(stdin);
        if(!scanf_s("%lf",&source[studentCount].medical.heightWeightPulsPressure._weight)) {puts("Input Error"); continue;}
        printf("Puls: "); fflush(stdin);
        if(!scanf_s("%d",&source[studentCount].medical.heightWeightPulsPressure._puls)) {puts("Input Error"); continue;}
        printf("Blood pressure: ");
        if(!scanf_s("%s",&source[studentCount].medical.heightWeightPulsPressure._bloodPressure)) {puts("Input Error"); continue;}
        break;
    }
}
...
void studentAdd(student * source)
{
    ...
    scanf_s("%s",&source[studentCount].personal.firstName);
    printf("Last name: "); fflush(stdin);
    scanf_s("%s",&source[studentCount].personal.lastName);
    printf("Second name: "); fflush(stdin);
    scanf_s("%s",&source[studentCount].personal.secondName);
    puts("-------------------------Medical data------------------------------");
    while (true) 
    {
    puts("Choose input type:\n1 - Height & weight only\n2 - Height, weight, puls and blood pressure\n");
    fflush(stdin); if (!scanf_s("%d", &source[studentCount].unionType)) {puts("Input Error"); continue;}
...
void studentDelete(student * source)
{
    ...
    printf("Your choise: ");
    fflush(stdin);
    if (!scanf_s("%d",&inp)) { puts("Input Error"); continue; }
